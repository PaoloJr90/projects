...
// rending the form for the user to add new-pet information


function AddPet() {

    // a state for each input field
    const [petName, setPetName] = useState()
    const [petType, setPetType] = useState()
    const [petAge, setPetAge] = useState()
    const [petBreed, setPetBreed] = useState()

    // build an object with the data from the form
    // this object will be in the body of the request
    const addPet = async () => {
        try {
            const petData = {
                name: petName,
            ...
            // check if the request was successful. If so, redirect to the pet detail page of the new pet (response.data.id).
